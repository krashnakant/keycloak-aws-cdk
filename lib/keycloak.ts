...
const KEYCLOAK_DOCKER_IMAGE_URI_MAP: dockerImageMap = {
    'aws': '323800856986.dkr.ecr.af-south-1.amazonaws.com/keycloak:latest:',
    ...
    private _generateKeycloakSecret(): secretsmanager.ISecret {
        return new secretsmanager.Secret(this, 'KCSecret', {
            generateSecretString: {
                ...
                passwordLength: 12,
                secretStringTemplate: JSON.stringify({username: 'keycloak1'}),
            },
        });
    }
}
...
export class ContainerService extends Construct {
    ...
    readonly repository = Repository.fromRepositoryName(this, 'KeycloakRepository', 'keycloak');
    ...
    constructor(scope: Construct, id: string, props: ContainerServiceProps) {
        ...
        const kc = taskDefinition.addContainer('keycloak', {
            image: ecs.ContainerImage.fromEcrRepository(this.repository, props.keycloakVersion.version),
            command: ['start --cache-config-file=cache-ispn-jdbc-ping.xml --hostname-strict-https=false --spi-connections-jpa-default-migration-strategy=update'],
            environment: Object.assign({
                KC_DB_URL_HOST: props.database.clusterEndpointHostname,
                KC_DB: 'mysql',
                KC_PROXY: 'edge',
                KC_DB_URL: `jdbc:mysql://${props.database.clusterEndpointHostname}:3306/keycloak`,
        ...
        kc.addPortMappings(
            ...
            {containerPort: 8080, protocol: ecs.Protocol.TCP,}, // HTTP web port
        );
